...
class VehiclePenaltyController extends Controller
{
    ...
    public function getNoPenalties() {
        ...
        // Filter out vehicles with 'Delivered' status in PHP (since it's an appended attribute)
        $datas = $vehicles->filter(function ($vehicle) {
            return $vehicle->delivery_status === 'Delivered';  // Only keep vehicles with delivered status
        });
        (new UserActivityController)->createActivity('Open No Penalty Vehicles Listing');
