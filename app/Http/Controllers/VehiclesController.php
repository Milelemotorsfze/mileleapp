        ...
        if ($request->ajax()) {
            ...
            if ($status === "allstock") {
                $data = Vehicles::select([
                    ...
                    'vehicles.booking_person_id',
                    'vehicles.purchasing_order_id',
                    // PO-side reserved salesperson (used to flag reservations that originate from the Purchase Order)
                    'purchasing_order.sales_person_id as po_sales_person_id',
                    'vehicles.vehicle_document_status',
