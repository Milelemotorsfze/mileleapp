    ...
    public function uploadingbulk(Request $request)
    {
        ...
        if ($rejectedCount > 0) {
            ...
            foreach ($leadsWithSalesPerson as $row) {
                ...
                $carInterested = isset($headerMap['CAR INTERESTED']) ? trim($row[$headerMap['CAR INTERESTED']] ?? '') : trim($row[12] ?? '');
                $purchasePurpose = isset($headerMap['PURPOSE OF PURCHASE']) ? trim($row[$headerMap['PURPOSE OF PURCHASE']] ?? '') : trim($row[13] ?? '');
                $endUser = isset($headerMap['END USER']) ? trim($row[$headerMap['END USER']] ?? '') : trim($row[14] ?? '');
                $destinationCountry = isset($headerMap['DESTINATION COUNTRY']) ? trim($row[$headerMap['DESTINATION COUNTRY']] ?? '') : trim($row[15] ?? '');
                $plannedUnits = isset($headerMap['PLANNED UNITS']) ? trim($row[$headerMap['PLANNED UNITS']] ?? '') : trim($row[16] ?? '');
                $experience = isset($headerMap['EXPERIENCE WITH UAE SOURCING']) ? trim($row[$headerMap['EXPERIENCE WITH UAE SOURCING']] ?? '') : trim($row[17] ?? '');
                $shipping = isset($headerMap['SHIPPING ASSISTANCE REQUIRED']) ? trim($row[$headerMap['SHIPPING ASSISTANCE REQUIRED']] ?? '') : trim($row[18] ?? '');
                $paymentMethod = isset($headerMap['PAYMENT METHOD']) ? trim($row[$headerMap['PAYMENT METHOD']] ?? '') : trim($row[19] ?? '');
                $prevPurchase = isset($headerMap['PREVIOUS PURCHASE HISTORY']) ? trim($row[$headerMap['PREVIOUS PURCHASE HISTORY']] ?? '') : trim($row[20] ?? '');
                $timeline = isset($headerMap['PURCHASE TIMELINE']) ? trim($row[$headerMap['PURCHASE TIMELINE']] ?? '') : trim($row[21] ?? '');
                $additionalNotes = isset($headerMap['ADDITIONAL NOTES']) ? trim($row[$headerMap['ADDITIONAL NOTES']] ?? '') : '';
                $csrPrice = isset($headerMap['CSR PRICE']) ? trim($row[$headerMap['CSR PRICE']] ?? '') : '';
                $csrCurrency = isset($headerMap['CSR CURRENCY']) ? trim($row[$headerMap['CSR CURRENCY']] ?? '') : '';
                $remarksArray = [];
