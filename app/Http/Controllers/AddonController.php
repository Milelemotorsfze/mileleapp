...
class AddonController extends Controller
{
    public function index($data)
    {
        ...
        return view('addon.index', compact(
            'addon1',
            'addonMasters',
            'brandMatsers',
            'modelLineMasters',
                ...
                if ($request->isAddonBoxView == 1) {
                    ...
                    if ($addon->is_all_brands == 'yes') {
                        ...
                        if ($content == '') {
                            ...
                            if (
                                $addon->LeastPurchasePrices->lead_time_max != ''
    ...
    public function create(Request $request)
    {
        ...
        return view('addon.create', compact(
            'addons',
            'brands',
            'modelLines',
            'suppliers',
    ...
    public function editAddonDetails($id)
    {
        ...
        return view('addon.edit.edit', compact(
            'addons',
            'brands',
            'modelLines',
            'addonDetails',
    ...
    {
        $supplierAddonDetails = [];
        $supplierAddonDetails = AddonDetails::where('id', $id)->with(
            'partNumbers',
            'AddonName',
            'AddonTypes.brands',
            'SellingPrice',
            'AddonSuppliers.Suppliers',
            'AddonSuppliers.Kit.addon.AddonName'
        )->first();
        $price = '';
    ...
    {
        // same permissions as the Accessories / Spare Parts / Kits / All menu links
        $listPermissions = ['P' => 'accessories-list', 'SP' => 'spare-parts-list', 'K' => 'kit-list'];
        if ($data != 'all' && !isset($listPermissions[$data])) {
            abort(404);
        }
        $user = Auth::user();
        $allowedTypes = array_keys(array_filter($listPermissions, fn ($permission) => $user->hasPermissionForSelectedRole([$permission])));
        if ($data != 'all') {
            $allowedTypes = array_intersect($allowedTypes, [$data]);
        }
        if (empty($allowedTypes)) {
            abort(403, 'You do not have permission to export addons.');
        }

        // hide the columns the addon listing hides for this role
        $hiddenColumns = [];
        if (!$user->hasPermissionForSelectedRole(['addon-least-purchase-price-view'])) {
            $hiddenColumns[] = 'Least Purchase Price (AED)';
        }
        if (!$user->hasPermissionForSelectedRole(['addon-selling-price-view'])) {
            $hiddenColumns[] = 'Selling Price (AED)';
        }
        if (!$user->hasPermissionForSelectedRole(['view-addon-supplier-info', 'supplier-addon-purchase-price-view'])) {
            $hiddenColumns[] = 'Suppliers';
        }

        $addons = AddonDetails::whereIn('addon_type_name', $allowedTypes)->with([
            'AddonName',
            'AddonDescription',
            'SellingPrice',
            'partNumbers',
            ...
            'AddonSuppliers.Suppliers',
            'AddonTypes.brands',
            'AddonTypes.modelLines',
        ])->orderBy('updated_at', 'DESC')->get();
        $names = ['P' => 'accessories', 'SP' => 'spare_parts', 'K' => 'kits', 'all' => 'addons'];
        (new UserActivityController)->createActivity('Exported ' . str_replace('_', ' ', $names[$data] ?? 'addons') . ' list');
        return Excel::download(new AddonsExport($addons, $hiddenColumns), ($names[$data] ?? 'addons') . '_' . now()->format('Y-m-d') . '.xlsx');
    }
}
