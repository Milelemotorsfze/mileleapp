...
<td>
    ...
    <td>{{ str_replace(['<p>', '</p>'], '', strip_tags($remarks)) }}</td>  
                    @php
                     $sales_notes = DB::table('lead_closed')->where('call_id', $calls->id)->first();
                     $sales_notes_closed = $sales_notes->sales_notes;
                     @endphp  
    <td>{{ $sales_notes_closed }}</td>    
                  </tr>
                @endforeach
              </tbody>
            </table>
