...
@php
  ...
  <div class="card-header">
    <h4 class="card-title">
     Calls & Messages Info (Rejected Leads)
    </h4>
    @php
...
<td>
    ...
    <td>{{ str_replace(['<p>', '</p>'], '', strip_tags($remarks)) }}</td>          
                  @php
    $rejectreasons = DB::table('lead_rejection')->where('call_id', $calls->id)->first();
    $reason = $rejectreasons ? $rejectreasons->Reason : null;
    $sales_notes = $rejectreasons ? $rejectreasons->sales_notes : null;
@endphp
