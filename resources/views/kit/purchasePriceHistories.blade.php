...
@section('content')
  ...
  @if ($hasPermission)
    <div class="card-header">
      <h4 class="card-title">
        Kit Purchase Prices History
      </h4>
      <a style="float: right;" class="btn btn-sm btn-info" href="{{route('addon.list','K')}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
    </div>
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table id="supplierAddonPrices" class="table table-striped table-editable table-edits table">
              ...
              <tbody>
                <div hidden>{{$i=0;}}</div>
                @foreach ($kitPriceHistories as $key => $historyData)
                  <tr data-id="1">
                    <td>{{++$i}}</td>
                    <td >{{$historyData->updated_price}} AED</td>
