            ...
            <div class="col-sm-4">
                <div class="row">
                    ...
                    <div class="col-sm-8">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input shipping_method @error('shipping_method') is-invalid @enderror" type="checkbox"
                                name="shipping_method" id="EXW" value="EXW" <?php echo ($quotation->shipping_method == 'EXW') ? 'checked' : ''; ?>>
            ...
            <div class="col-sm-4">
                <div class="row">
                    ...
                    <select class="form-select" name="currency" id="currency">
                        ...
                        <option <?php echo ($quotation->currency == 'EUR') ? 'selected' : ''; ?>>EUR</option>
                        <option <?php echo ($quotation->currency == 'PHP') ? 'selected' : ''; ?>>PHP</option>
                    </select>
                </div>
                </div>
            </div>
            ...
            <div class="col-sm-4">
                <div class="row mt-2">
                    ...
                    <select class="form-control" id="client_category" name="client_category">
                        <option value="Individual" {{ ($callDetails->client_category == 'Individual') ? 'selected' : '' }}>Individual</option>
                        <option value="Company" {{ ($callDetails->client_category == 'Company') ? 'selected' : '' }}>Company</option>
                    </select>
                </div>
                </div>
                <div class="row mt-2" id="contact-person-div" hidden>
                    ...
                    <div class="col-sm-6">
                        <input type="text" class="form-control form-control-xs" name="contact_person" id="contact-person" placeholder="Contact Person" value="{{$callDetails->client_contact_person}}">
                    </div>
                </div>
                <div class="row mt-2" id="company-div" hidden>
                    ...
                    <div class="col-sm-6">
                        <input type="text" class="form-control form-control-xs" name="company_name" id="company" placeholder="Company Name" value="{{$callDetails->company_name}}">
                    </div>
                </div>
                <div class="row mt-2">
                    <div class="col-sm-6">
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-sm-10">
                        <input type="text"  class="form-control form-control-xs "
                               name="remarks" placeholder="Remarks" value="{{$quotation->sales_notes}}">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    var selectedAgentId = "{{ $quotation_details->agents_id ?? '' }}";
    var selectedCbName = "{{ $quotation_details->cb_name ?? '' }}";
    var selectedCbNumber = "{{ $quotation_details->cb_number ?? '' }}";
    ...
    $(document).ready(function () {
        ...
        // Fetch and initialize agent data
        fetchAgentData();
        ...
        function fetchAgentData() {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Set selected values if in edit mode
                    if (editingMode) {
                        $('#cb_name').val(selectedAgentId).trigger('change');
                        $('#cb_number').val(selectedCbNumber).trigger('change');
                    }
                },
                error: function (error) {
                    console.error('Error fetching agent data:', error);
        ...
        $('#form-update2_492').submit(function (e) {
            ...
            $.ajax({
                ...
                contentType: false,
                success: function (response) {
                    // If in edit mode, update the selected agent values
                    if (editingMode) {
                        $('#cb_name').empty().append('<option value="' + response.agent_id + '">' + response.name + '</option>');
                        $('#cb_name').val(response.agent_id).trigger('change');
                        $('#cb_number').val(response.phone).trigger('change');
                    } else {
                        // If in create mode, append a new option
                        $('#cb_name').append('<option value="' + response.agent_id + '">' + response.name + '</option>');
                    }
    ...
    targets: -1,
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    // calculate — initial line total only (whole number in amount field)
                    var quantityValue = row.qty ? row.qty : 1;
                    var amount = Math.round(parseFloat(price) * parseFloat(quantityValue)) || 0;
        ...
        return `<input type="number" min="0" value="${quantityValue}" step="1" 
                ...
                },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if(row['button_type'] == 'Vehicle') {
                        ...
                        if(row['edit_page'] == 'editpage')
                        {
                        combinedValue =  row[1] + comma1 + row[2] + comma2 + row[3]+ comma3 + row[5] + comma4 + row[4];
                        }
                        else
                        {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    // Existing items already store unit_price in the quotation's currency, so they
                    // must be shown as-is. Only newly added items (whose source price is in AED)
                    // get converted to the selected currency, matching the create form.
                    if (row['edit_page'] != 'editpage') {
                        if (currency == 'USD') {
...
function updateSecondTable(RowId, savedVins) {
    ...
    $('#country').change(function() {
        ...
        if (countryId) {
            ...
            $('#shipping_port').prop('disabled', false).empty().html('<option value="" disabled selected>Select Shipping Country</option>');
            $.ajax({
