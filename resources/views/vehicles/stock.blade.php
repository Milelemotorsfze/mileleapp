    ...
    columnMap[40] = 'vehicles.custom_inspection_status';
        ...
        var table7 = $('#dtBasicExample7').DataTable({
          processing: true,
            serverSide: true,
            columns: columns7,
            order: window.stockReportOrderByPoDate(columns7),
            ajax: {
...
function exportToExcel(tableId) {
    if (typeof window.exportStockReportToCsv === 'function') {
        window.exportStockReportToCsv(tableId, 'stock-report-export.csv');
        return;
    }
    if (typeof alertify !== 'undefined') {
        alertify.error('Export is not available. Please refresh the page.');
    }
}
