...
<div class="card">
    <div class="card-header">
        ...
        @if ($hasPermission)
            <h4 class="card-title">
                @if(isset($type) && $type == 'export_exw') Export EXW @elseif(isset($type) && $type == 'export_cnf') Export CNF @elseif(isset($type) && $type == 'local_sale') Local Sale @endif Work Order Vehicle PDI Status History
            </h4>
                ...
                <table class="table table-striped table-editable table-edits table-condensed my-datatableclass">
                    <thead style="background-color: #e6f1ff">
                        <tr>
                            ...
                            <th>Status</th>
                            <th>Comment</th>
                            <th>PDI Scheduled At</th>
                            <th>Passed/ Failed</th>
                            <th>QC Inspector Remarks</th>
                            <th>Updated By</th>
                            <th>Updated At</th>
                        </tr>
                    </thead>
                        ...
                        @if(count($data) > 0)
                            ...
                            @foreach($data as $one)
                                <tr data-id="{{ $one->id }}">
                                    <td>{{ ++$i }}</td>
                                    <td>
                                        <label class="badge @if($one->status == 'Scheduled') badge-soft-info @elseif($one->status == 'Completed') badge-soft-success @elseif($one->status == 'Not Initiated') badge-soft-danger @endif">{{ $one->status ?? ''}}</label>
                                    </td>
                                    <td>{{ $one->comment ?? '' }}</td>
                                    <td>
                                        @if(!empty($one->pdi_scheduled_at))
                                            {{ \Carbon\Carbon::parse($one->pdi_scheduled_at)->format('d M Y, h:i:s A') }}
                                        @endif
                                    </td>                                     
                                    <td>
                                        <label class="badge @if($one->passed_status == 'Passed') badge-soft-success @elseif($one->passed_status == 'Failed') badge-soft-danger @endif">{{ $one->passed_status ?? ''}}</label>
                                    </td>
                                    <td>{{$one->qc_inspector_remarks ?? ''}}</td>
                                    <td>{{ $one->user->name ?? '' }}</td>
                                    <td>
