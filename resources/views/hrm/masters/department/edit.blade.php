...
@section('content')
@php
$hasPermission = Auth::user()->hasPermissionForSelectedRole(['edit-department']);
@endphp
@if ($hasPermission)
<div class="card-header">
	<h4 class="card-title">Edit Department Details</h4>
	@if($previous != '')
	<a  class="btn btn-sm btn-info float-first" href="{{ route('department.edit',$previous) }}" ><i class="fa fa-arrow-left" aria-hidden="true"></i> Previous Record</a>
	@endif
	@if($next != '')
	<a  class="btn btn-sm btn-info float-first" href="{{ route('department.edit',$next) }}" >Next Record <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
	@endif 
...
<div class="card-body">
	...
	@endif
	<form id="editDivisionForm" name="editDivisionForm" enctype="multipart/form-data" method="POST" action="{{route('department.update',$data->id)}}">
		@csrf
		@method("PUT")
		<div class="row">
				...
				<div class="row">
					...
					<div class="col-xxl-6 col-lg-6 col-md-6 select-button-main-div">
						<div class="dropdown-option-div">
							...
							<option value="{{$approvalBy->id}}" @if($data->approval_by_id == $approvalBy->id) selected @endif>{{$approvalBy->name}}</option>
							@endforeach
							</select>
						</div>
					</div>
