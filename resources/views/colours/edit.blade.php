...
<div class="card-header">
    ...
    <a class="btn btn-sm btn-info float-end" href="{{ route('colourcode.index') }}"><i class="fa fa-arrow-left"></i> Back</a>
</div>
    ...
    <form id="form-update" action="{{ route('colourcode.update', $colorcodes->id) }}" method="POST">
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <input type="text" id="name" name="name" class="form-control" placeholder="Color Name" value="{{ old('name', $colorcodes->name) }}" required>
            ...
            <div class="col-md-4">
                ...
                <select class="form-control" name="belong_to" required>
                    <option value="int" {{ $colorcodes->belong_to == 'int' ? 'selected' : '' }}>Interior</option>
                    <option value="ex" {{ $colorcodes->belong_to == 'ex' ? 'selected' : '' }}>Exterior</option>
                </select>
            </div>

            <input type="hidden" id="original_name" value="{{ $colorcodes->name }}">
            <input type="hidden" id="original_belong_to" value="{{ $colorcodes->belong_to }}">
                ...
                <div class="row">
                    ...
                    <div class="input-group d-flex flex-nowrap parent_color_name">
                        <select class="form-control select2" id="parent_colour_id" name="parent_colour_id" style="width: 100%" multiple>
                            <option disabled>Select Parent Colour</option>
                            @foreach($parentColours as $parentColour)
                            <option value="{{ $parentColour->id }}" {{ $colorcodes->parent_colour_id == $parentColour->id ? 'selected' : '' }}>
                                {{ $parentColour->name }}
                            </option> @endforeach
                        </select>
        ...
        <div class="row mt-3">
            <div class="col-md-4">
                ...
                <div class="d-flex flex-column">
                    <span>
                        <input type="radio" id="color_code_yes" name="color_code_required" value="yes" checked>
                        <label for="color_code_yes">Yes</label>
                    </span>
                    <span>
                        <input type="radio" id="color_code_no" name="color_code_required" value="no">
        ...
        <div id="color-code-section" class="row col-12 mt-3">
            <label for="color_codes"><strong>Color Codes</strong><span class="error"> *</span>
                <button type="button" class="btn btn-success add-color-code add-color-code-main-button">Add Color Code</button>
            </label>
            <div id="color-code-container" class="col-xxl-4 col-md-4 col-sm-6 col-12">
                @foreach($colorcodes->dpColorCodes as $dpColor)
                <div class="color-code-div-container d-flex align-items-center col-6">
                    <div class="input-group mb-2">
                        <input type="text" name="color_codes[]" class="form-control" placeholder="Enter Color Code" value="{{ $dpColor->color_code_values }}" required>
...
<script>
    $(document).ready(function() {
        ...
        function fetchParentColors() {
            let selectedParentColors = $('#parent_colour_id').val();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(function(color) {
                        let isSelected = selectedParentColors && selectedParentColors.includes(color.id.toString()) ? 'selected' : '';
                        $('#parent_colour_id').append(`<option value="${color.id}" ${isSelected}>${color.name}</option>`);
                    });
                    $('#parent_colour_id').select2({
                        placeholder: "Select Parent Colour",
                        maximumSelectionLength: 1
                    });
                    $('#parent_colour_id').val(selectedParentColors).trigger('change');
...
<script>
    ...
    function autoSelectColorCodeRequired() {
        let codes = $('input[name="color_codes[]"]').map(function() {
            return $(this).val().trim();
        }).get();

        let hasValidCode = codes.some(code => code.length > 0);

        if (hasValidCode) {
            $('#color_code_yes').prop('checked', true);
        } else {
            $('#color_code_no').prop('checked', true);
        }
    }

    $(document).ready(function() {

        autoSelectColorCodeRequired();

        $(document).on('input', 'input[name="color_codes[]"]', function() {
            autoSelectColorCodeRequired();
        });

        $(document).on('click', '.remove-color-code', function() {
            setTimeout(autoSelectColorCodeRequired, 100);
        });
        ...
        function checkNameAvailability(name) {
            let originalName = $('#original_name').val().trim();
            let originalBelongTo = $('#original_belong_to').val();
            let currentBelongTo = $('select[name="belong_to"]').val();

            if (name === originalName && currentBelongTo === originalBelongTo) {
                $('#nameError').text('');
                $('#form-update').find(':submit').prop('disabled', false);
                return;
            }
            ...
            if (!validateColorName(name)) {
                ...
                $('#form-update').find(':submit').prop('disabled', true);
                return;
            }
